...
public class Tabla {
    ...
    public void Filtra2() {
        Filtro.sendKeys(ReadProperties.readFromConfig("Propiedades.properties").getProperty("TXTcampo1"));
        BotonFiltrar.click();
    }
        public void Obtener(){
    ...
    public String[] recorrer2(){
        ...
        ArrayList<ArrayList<String>> tabla = new ArrayList(rows.size());
        List<WebElement> filas = Tabla.findElements(By.tagName("tr"));
        String [] Rowsstring = new String[rows.size()];
        for (int i=0; i< rows.size(); i++){
            ArrayList<String> datosfila = new ArrayList(cols.size());
            List<WebElement> fila = filas.get(i).findElements(By.tagName("td"));
            String reg = "";
